...
export default {
    ...
    methods: {
        ...
        getOption:function(dataOption){
           ...
           this.option.yAxis[0].name = yAxisName.yAlias;
           this.option.yAxis[0].data = yAxisName.yName;
           this.option.series = [];
           for(let i=0; i<seriesName.length; i++){
               this.option.series.push({name:seriesName[i].xAlias,data:seriesName[i].xName,type: 'bar'})
           }
           return this.option;
          
       },
       attachData:function(eoption,data){
            util.attachData(eoption,data,'HBar',0);
        },
       reset:function(){
           let Vue = this;
           Vue.option.color = 0;
           Vue.option.yAxis.data= '',
           Vue.option.yAxis.name= '',
